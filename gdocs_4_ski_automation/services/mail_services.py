...
def send_mail(to_email, template,mail_settings_dir,credentials_dir):
    ...
    try:
        ...
        yag.send(subject="Great!",contents=template['body'],to=to_email,attachments=template['attachments'])
...
def fill_registration_template(registration:Registration, _template,checklist_dir):
    ...
    for p in registration.participants:
        _participants.append({'first_name': p.name.first, 'last_name': p.name.last, 'age': p.age, 'course': p.course.value})
...
def mail_service(registrations,paid_template_dir,registration_template_dir,mail_settings_dir,credentials_dir,checklist_dir="data/mails/checklist.pdf",send_mail_function=send_mail_dummy,):
...
if __name__ == "__main__":
    ...
    factory = GDocsRegistrationFactory("data/dependencies/client_secret.json", {"settings": "1SteMGOoigoPyZMJsB5GG82K4WNh-N2AQIck_xtrmtz8", "registrations": "11FLy4qTOScUOLj4xGVPMy12940DsOYz2pmoPDf9pDhA", "db": "1VUuX4UbsWsxd5QUKA7FjsebU2uPTs80dBSSn4Tay_Vk"})
    registrations = factory.build_registrations()
    mail_service(registrations[:1],"data/mails/payed.html","data/mails/registration.html","data/dependencies/mail_setting.yaml","data/dependencies/client_secret_mail.json","data/mails/checklist.pdf",send_mail)